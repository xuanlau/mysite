...
def lang_chao():
    for ip in range(42, 48):
        ...
        time.sleep(5)
        browser.find_element(By.ID, 'userid').send_keys('admin')
        ...
        alert.accept()
        time.sleep(8)
        # browser.find_element(By.CLASS_NAME, 'navbar-btn sidebar-toggle').click()
        a = browser.find_elements(By.TAG_NAME, 'h4')
        # for i in a:
        #     info = i.text
        #     print(i.text)
        #     if info == '系统信息':
        ...
        a[4].click()
        time.sleep(8)
        b = browser.find_elements(By.TAG_NAME, 'a')

        b[-2].click()
        time.sleep(1)
        c = browser.find_element(By.XPATH, '//*[@id="device-content"]/div/div[1]/div/div/div[2]/table/tbody/tr')
        d = browser.find_element(By.XPATH, '//*[@id="device-content"]/div/div[2]/div/div/div[2]/table/tbody')
        print(c.text)
        print(d.text.split())
        # browser.quit()
...
def bing_sousuo():
    browser.get('https://search.1688.com/company/pc/factory_search.html?charset=utf8&hideMainTab='
                '1&keywords=%E5%A5%B3%E8%A3%85&pagesource=sem_93d9f5b217df9341a33862be481e5945a50f')
    time.sleep(2)
    try:
        browser.find_element(By.ID, 'fm-login-id').send_keys('dadad')
        browser.find_element(By.ID, 'fm-login-password').send_keys('dadada')
    except:
        print('xxxx')
    company_list = browser.find_element(By.CLASS_NAME, 'company-original-offer-list').find_elements(By.CLASS_NAME,
                                                                                                    'title')
    for i in company_list:
        print(i.text)
    # browser.find_element(By.ID, 'alisearch-submit').click()
